...
We hypothesize the following about the fungal amylase used in lab:
\begin{itemize}
    ...
    rate when both amylase and starch are present.

    \item Amylase will convert starch to maltose faster at \degreesC{80} than at any other temperature.
\end{itemize}
...
\degreesC{80} as taking 750 seconds, on average, to digest \SI{2}{mL} 0.5\% starch solution --
