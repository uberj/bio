...
He hypothesis the following about the fungal amylase used in lab:
\begin{itemize}
    \item Solutions with a higher concentration of amylase will consume starch in a solution
    faster than a solution with a lower concentration of amylase.

    \item Solutions with a higher pH level will facilitate a faster starch to maltose conversion
    rate when both amylase and start are present.

    \item Amylase will convert starch to maltose the fastest when the environmental temperature is
\degreesC{80}. \\
\end{itemize}
...
% Use subsections
    % * Iodine paragraph
An iodine (\ce{I_{2}KI}) test was used to test all three hypothesis.

\subsection{Iodine Test}
\label{sec:iodine-test}
An iodine test involves a test plate holding \SI{10}{mL} wells 6 rows wide and 10 columns deep.
...
created by adding one of the tube's solution to the other. The observation interval was
\SI{10}{seconds}. \\
...
the longest digestion time and that an environment at \degreesC{37} took the shortest amount of
time. The solution in a \degreesC{80} water bath, on average, took 750 seconds to digest all of its
starch which was the slowest digestion time. \\
...
that strictly increased as concentration of the enzyme was increased. When the amylase concentration
was at its highest (50\%), amylase took the least amount of time to digest all the starch \\
...
We hypothesised that solutions with a higher concentration of amylase would consume all starch in a
solution faster than a solution with a lower concentration of amylase. From the Amylase
...
We also hypothesised that solutions with a higher pH level would facilitate a faster starch to maltose
...
Finally, we hypothesised that amylase would convert starch to maltose the fastest when the
environmental temperature is \degreesC{80}. We can see that Table~\ref{temp-table} lists
\degreesC{80} as taking 750 seconds, on average, to digest \SI{2}{mL} 0.5\% of starch solution --
this was the longest digestion time we observed. Since other temperatures digested their starch in
less time than \degreesC{80}, our data seems to refute our hypothesis. Specifically \degreesC{37}
took only 201 seconds, on average, to digest the starch in its environment which directly contradicts
our hypothesis since \degreesC{80} took 750 seconds.
