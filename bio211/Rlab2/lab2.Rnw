...
%Partners: & Jacques \\ % Partner names
%& Chris \\
%& Daniel \\
%& Itzel \\
...
Enzyme shape is influenced by the precense or absense of \ce{OH-} or \ce{H+} ions. These ions
interact with the amino acid side chains of the enzyme, which influence its secondary, tertiary, and
in some cases, quatrinary structure. The pH an enzyme is most effective at is called
...
Amylase is an enzyme that that plays a crucial role in animals as an enzyme that helps break down
starch during digestion. Some amylase, like \aamylase, is used in industrial processes as a starch
liquefaction agent and as a detergent. Starch is made up of two polysaccharides: amylose and
amylopectin. Amylose is comprised of \aglucose monomers connected via a \onefour-glycosidic bond.
Amylopectin is made of similar \aglucose monomers but is more branched than amylose due to
occasional \onesix-glycosidec bonds between two glucose. All amylase enzymes can perform glycolysis,
cleaving \onefour-glycosidic bonds. \\

In humans, certain types of \aamylase is present as a food digestive enzyme in the intestinal luman.
The enzyme helps to curve excesive digestion and consumes glucose before it can enter the intestinal
...
catalyst. \Aamylase is especially good at resisting denaturing at a broad range of temperatures
...
For our experiments we were given an unknown fungal amylase. Fungal amylase is similar to \aamylase
in that it cleaves \onefour glycosidic bonds. We seek to investigate the influences pH, temperature,
...
conversion rate when both amylase and start were present. Looking at the pH graph in
...
in~\ref{sec:introduction}, some \aamylase used in industrial processes are active up to pH 11.5. Our
fungal amylase would not be a good amylase for use in extreme industrial processes that require
activity at high end pH levels. On the contrary, our optimal pH was actually very similar to that of
...
was the longest digestion time we observed. Since other temperatures digested their starch in less
time than \degreesC{80}, our data seems to refute our hypothesis. Specifically \degreesC{37} took
only 201 seconds, on average, to digest the starch in its environment which directly contradicts our
hypothesis. \\

We observed that every experimental replicant had to stop the continuation of the iodine
test after 600 seconds -- the fungal amylase seemed to denature. It is interesting to note that some
\aamylase used in industrial processes discussed in~\ref{sec:introduction} have a much higher
thermostability than the fungal amylase used in our experiment. The most productive temperature we
tested our fungal amylase at was \degreesC{37}.  \Aamylase from \emph{Saccharomycopsis egfibuligera}
has an optimum temperature range from \degreesC{40} to \degreesC{50} which is similar to our
results \cite{Hostinova}. \\
...
\subsection{Error}
We had five replicants for each experiment performed. Each replicant was carried out by a different
experimenter and so experimental bias and experimental error could have occurred in many areas For
example, the iodine test relies on a subjective decision of whether the iodine solution is "blue".
Also, each group doing the experiment could have different ideas of what "blue" was. This type of
experimental error would seem to be very hard to measure. \\
...
data point. Floating point division was used both these cases.
